...
selector SHIPPED ahead of its competitive arena verdict (`742e3c4a`, cache v38): its value is long-term
positional against humans, which a 3 s self-play arena cannot measure, so it goes to a human playtest;
the arena number is recorded in FINDINGS when in.
