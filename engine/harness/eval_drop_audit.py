...
def main():
    ...
    # the same pass. Running A alone over RECORDED games silently reproduces
    # the contaminated numbers, so that combination has to be asked for
    # explicitly.
    #
    # SELF-PLAY IS EXEMPT, and the first version of this guard wrongly blocked
    # it. There every half-move was generated by the engine that played it, so
    # no window can cross a turn no legal play produces -- `do_reach` is False
    # for exactly that reason. Demanding Check B there would refuse the one
    # source that needs no filter.
    filter_corrupt = (not args.no_corrupt_filter) and 'a' in args.checks \
                     and do_reach
    if filter_corrupt and 'b' not in args.checks:
        sys.exit("CHECK A over recorded games without CHECK B cannot exclude "
                 "corrupt windows, and 37.9% of the first run's flags were "
                 "corrupt windows. Use --checks ab (one hydration serves "
                 "both), or --no-corrupt-filter to reproduce the contaminated "
                 "numbers on purpose.")
    ...
    if filter_corrupt:
        ...
        print("  corrupt-window filter N/A: every half-move in self-play was "
              "generated by the engine that played it, so no window can cross "
              "a turn no legal play produces")
    else:
