...
// DISCARDED (2026-09-20): exhaustive mate-in-1 "bookends" around the search.
// Engine v6/v7 scanned the root's FULL enumeration for an immediate win before
// searching and the chosen move's full reply list for an immediate loss after
// (banning the move and re-searching). They fixed the recorded "-0.5 then
// mated in one" games, but a 3 s fleet arena measured the pre-pass+bookends
// package at 38.4% (-82 Elo [-97, -67]) against the v5 search: the scans (up to
// 250k turns, twice per move) ran outside the time control, so a 3 s budget
// averaged 4.8 s per move while the search itself got LESS time. The
// stone-lead pre-pass in `turn_iter.rs` finds 95% of the same mates for
